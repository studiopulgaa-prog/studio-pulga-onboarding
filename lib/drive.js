...
const FOLDER_ID_PADRAO = '1M4uhyUFKK8FqIhr-S0XVeYQ8n1n6TgVI';
...
async function pastaExiste(token, id) {
...
async function obterPastaRaiz(token) {
  const salva = await rpc('sp_cfg_get', { p_chave: 'drive_folder_raiz' });
  const candidato = salva || FOLDER_ID_PADRAO;
  if (await pastaExiste(token, candidato)) return candidato;
  // A pasta configurada nao existe ou nao e acessivel pela conta conectada:
  // cria uma pasta raiz nova na conta atual e passa a usar ela dali em diante.
  const r = await fetch('https://www.googleapis.com/drive/v3/files?fields=id', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    body: JSON.stringify({ name: 'Formularios Clientes Studio', mimeType: 'application/vnd.google-apps.folder' })
  });
  const t = await r.text();
  if (!r.ok) throw new Error(`Nao foi possivel criar a pasta raiz no Drive ${r.status} ${t}`);
  const novaRaiz = JSON.parse(t).id;
  await rpc('sp_cfg_set', { p_chave: 'drive_folder_raiz', p_valor: novaRaiz });
  // Torna a pasta acessivel por link, para eliminar duvida de qual conta acessar
  try {
    await fetch(`https://www.googleapis.com/drive/v3/files/${novaRaiz}/permissions`, {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
...
async function criarPasta(token, nome, folderRaiz) {
  const r = await fetch('https://www.googleapis.com/drive/v3/files?fields=id', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    body: JSON.stringify({ name: nome, mimeType: 'application/vnd.google-apps.folder', parents: [folderRaiz] })
  });
  const t = await r.text();
...
async function syncEnvio(id) {
  ...
  try {
    ...
    const folderRaiz = await obterPastaRaiz(token);
    const pasta = row.drive_folder_id || await criarPasta(token, `${empresa} ~ ${dataBR(row.criado_em)}`, folderRaiz);
